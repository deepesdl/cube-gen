...
def chunk_and_create_encoding_dict(ds, time_chunking, lat_chunking, lon_chunking, quiet=True):
    
    for var_name in ds.data_vars:
        if var_name != "crs":
            if not quiet:
                print(var_name)
            ds[var_name] = ds[var_name].chunk({'time': time_chunking, 'lat': lat_chunking, 'lon': lon_chunking})
            
    encoding_dict = dict()
    
    coords_encoding = {k: dict(chunks=v.shape) for k, v in ds.coords.items()}
    vars_encoding = {k: dict(chunks=(time_chunking, lat_chunking, lon_chunking), write_empty_chunks=False) for k, v in ds.data_vars.items() if k != "crs"} 
...
def save_site(site):
    ...
    ds, encoding_dict = chunk_and_create_encoding_dict(ds, time_chunking, lat_chunking, lon_chunking, True)
    ...
    store_output.write_data(
        ds,
        f"hydrology-S{site}-0.009deg-{time_chunking}x{lat_chunking}x{lon_chunking}-1.0.0.zarr",
        replace=True,
