...
for size in [200,300]:
    ...
    pathIn = f"~/data/polar/source/geothermal_heat_flow_{size}x{size}km_xyz_data"
    ...
    filepath = os.path.join(pathIn,f'{size}km_GHF_windowCenters.nc')

    da = regrid(filepath)

    da.name = f"geothermal_heat_flow_{size}km"
    ...
    da.to_zarr(f"{pathOut}/geothermal_heat_flow_{size}km.zarr")